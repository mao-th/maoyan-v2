<template>
  <div id="address-wrap">
    <div class="address">
      <div class="cinema-name" v-text="nm"></div>
      <div class="address-name" v-text="addr"></div>
    </div>
    <div class="icon-addr">
      <img
...
export default {
  ...
  computed: {
    addr() {
      return this.cinemaData.addr;
    },
    nm() {
      return this.cinemaData.nm;
    },
    // 后续可在百度地图上应用
    lat() {
      // 当前电影院经度
      return this.cinemaData.lat;
    },
    lng() {
      // 当前电影院纬度
      return this.cinemaData.lng;
    }
  }
};
</script>

<style lang="scss" scpted>
#address-wrap {
  display: flex;
